...
class Recent_Posts_Widget_With_Thumbnails extends WP_Widget {
	...
	function __construct() {
		...
		$this->defaults[ 'plugin_version' ]		= '6.8.0'; // current plugin version
		...
		$this->defaults[ 'nav_label' ]			= __( 'Recent Posts With Thumbnails' ); // ARIA label of the navigation list
	...
	function widget( $args, $instance ) {
		...
		if ( $r->have_posts()) :
			...
			// let there be an empty author label if desired
			if ( isset( $instance[ 'author_label' ] ) ) {
				if ( '' === $instance[ 'author_label' ] ) {
					$this->customs[ 'author_label' ] = '';
				} else {
					$this->customs[ 'author_label' ] = $instance[ 'author_label' ];
				}
			} else {
				$this->customs[ 'author_label' ] = $this->defaults[ 'author_label' ];
			}
	...
	function update( $new_widget_settings, $old_widget_settings ) {
		...
		// let there be an empty author label if desired
		if ( isset( $new_widget_settings[ 'author_label' ] ) ) {
			if ( '' == $new_widget_settings[ 'author_label' ] ) {
				$instance[ 'author_label' ] = '';
			} else {
				$instance[ 'author_label' ] = $new_widget_settings[ 'author_label' ];
			}
		} else {
			$instance[ 'author_label' ] = $this->defaults[ 'author_label' ];
		}
	...
	function form( $instance ) {
		...
		if ( isset( $instance[ 'author_label' ] ) ) {
			...
			$author_label = $this->defaults[ 'author_label' ];
		}
